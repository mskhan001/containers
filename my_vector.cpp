...
public:
    ...
    // default constructor
    myvector_base(size_type n, const A &alloca = A()) : alloca_(alloca),
                                                        elem_(alloca_.allocate(n)),
                                                        last_(elem_ + n),
                                                        space_(elem_ + n)
    {
    }
    ...
    myvector_base &operator=(const myvector_base &) = delete; // delete copy assignment

    // move constructor
    myvector_base(myvector_base &&oth) : alloca_(std::move(oth.alloca_)),
                                         elem_(oth.elem_),
                                         last_(oth.last_),
                                         space_(oth.space_)
    {
    ...
    ~myvector_base()
    {
        alloca_.deallocate(elem_, last_ - elem_);
    }
...
public:
    ...
    // default constructor
    explicit myvector(size_type n, const T &init_val = T(), const A &alloca = A()) : vb_(n, alloca)
    {
    ...
    {
        myuninitialized_copy<decltype(vb_.elem_), T>(oth.vb_.elem_, oth.vb_.last_, vb_.elem_);
    }
    ...
    myvector &operator=(const myvector &oth)
    {
        ...
        if (new_size > size())
        {
            ...
            myuninitialized_copy(oth.vb_.elem_ + size(), oth.vb_.last_, vb_.elem_ + size());
        }
        else
        {
    ...
    {
        destroy_range(vb_.elem_, vb_.elem_ + size());
    }
